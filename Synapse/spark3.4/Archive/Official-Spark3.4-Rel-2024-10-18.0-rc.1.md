...
*   **Vhd Release**: Spark3.4-Rel-2024-10-18.0-rc.1
*   **Operating System**: CBL-Mariner 2.0.20241006
*   **Apache Spark**: 3.4.3
*   **Java**: 11.0.24
*   **Scala**: 2.12.17
*   **Python**: 3.10/2.0.0
...
|-----|-----|-----|
|1471387|NotebookUtils:1.1.55|Support notebookutils.udf.run in Fabric, fix Mount issue in Gov Cloud, bug fix to address unexpected display results of custom class objects in JSON conversion, fix display ploars dataframe missing column name, and support redacting spark conf|
...
|Id|Component|Description|
|-----|-----|-----|
|1487541|Wildfire:1.9.20, OnelakeSparkCatalog:0.1.16|Handle catalog initialization during Delta's Merge Into operation when lakehouse is not attached, other oss backports in hadoop.|
|1483591|LibraryManager:1.0.20|Add support for migration of system storage account WASB to ABFSS and add support for Fabric LM to allow customers to specify python libraries from custom channels setup on their storage account|
|1450664|MMLSpark:1.4.18|Updated the SynapseML distributed machine learning framework libraries shipped as part of Fabric VHD component https://github.com/microsoft/SynapseML/releases/tag/v1.0.7.|
|1478918|ServiceConfigurationTemplates|For HC sessions, in the driver and executor logs add the Notebook Id corresponding to the log lines.|
|1481008|SparkRPCHistoryServer:1.3.0|To update RPCHS to 1.3.0-20240821.1 so as to pick up the following changes since last release (1.2.0-20240821): Improvement: Add new log metadata APIs to enhance the response contract, which are used by Marlin FE unified router. And ventually, used by Fabric Public API. No change made on existing logic.  Also a bug fix: for Spark UI, sort the loaded plugins by their displayOrder to ensure a consistent order.|
|1478564|Wildfire/spark34:1.9.19|Add support for creating temporary and global temporary views when lakehouse is not attached. Add delta to special list of single part of allowed namespaces.|
|1476992|NotebookUtils:1.1.56|Update Notebookutils to 1.1.5 to be compatible with illegal spark.fabric.environmentDetails format|
|1471807|Wildfire/spark34:1.9.18|New Spark 3.4 component release, including improvements and fixes Livy and Bigtop|
|1471387|NotebookUtils:1.1.55|Support notebookutils.udf.run in Fabric, fix Mount issue in Gov Cloud, bug fix to address unexpected display results of custom class objects in JSON conversion, fix display ploars dataframe missing column name, and support redacting spark conf|
...
|-----|-----|-----|
|1487541|Wildfire:1.9.20, OnelakeSparkCatalog:0.1.16|Handle catalog initialization during Delta's Merge Into operation when lakehouse is not attached, other oss backports in hadoop.|
|1480512|CSparkMetricUpdate:1.0.8|Revert coding sign jars. A difference in security certs causes exceptions, failing spark jobs.|
|1481008|SparkRPCHistoryServer:1.3.0|To update RPCHS to 1.3.0-20240821.1 so as to pick up the following changes since last release (1.2.0-20240821): Improvement: Add new log metadata APIs to enhance the response contract, which are used by Marlin FE unified router. And ventually, used by Fabric Public API. No change made on existing logic.  Also a bug fix: for Spark UI, sort the loaded plugins by their displayOrder to ensure a consistent order.|
|1479884|OnelakeSparkCatalog:0.1.14|Enables support for spark sql on file on Delta format table paths, e.g. `select * from delta.abfss://...`, fixes bug in creating or replacing global or temporary view using spark sql when no lakehouse is attached, fixes bug in case insensitive schema name resolution|
|1471387|NotebookUtils:1.1.55|Support notebookutils.udf.run in Fabric, fix Mount issue in Gov Cloud, bug fix to address unexpected display results of custom class objects in JSON conversion, fix display ploars dataframe missing column name, and support redacting spark conf|
|1474485|TokenLibrary:4.3.4|Enable System SasToken Refresh for Synapse and non-Spark Java Consumers|
...
|decorator|5.1.1|**prompt-toolkit**|**3.0.42 ⬆️ 3.0.48**|
...
|giflib|5.2.1|**python**|**3.10.13 ⬆️ 3.10.15**|
...
|hk2-locator|2.6.1|**spark-hive**|**2.12-3.4.3.5.3.20240904.5 ⬆️ 2.12-3.4.3.5.3.20241016.1**|
