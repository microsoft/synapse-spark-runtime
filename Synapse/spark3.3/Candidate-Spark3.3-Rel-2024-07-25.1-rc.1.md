# System Environment
*   **Vhd Release**: Spark3.3-Rel-2024-07-25.1-rc.1
...
|-----|-----|-----|-----|
|1360734|ServiceConfigurationTemplates/spark/3.3.0:various|Enable support of spaces in table name by default for Hive|Adds the configuration to allow spaces in table names. The config is added only for Hive. user needs to set the spark config (spark.sql.valid.characters.in.table.name ) to be able to create table having spaces in name.|
|1396780|TridentTokenLibrary:1.9.4|SPN + Certificate based authentication|New commands enabling SPN + Certificate based authentication|

# Improvements

|Id|Component|Title|Description|
|-----|-----|-----|-----|
|1401486|Impulse:1.2.4|Upgrade Impulse Version to 1.0.22|Upgrade Impulse Version to 1.0.22. Changes included: Add delta error classifier, Component Governance for all SubProjects, Impulse Infrastructure Improvements, Update Impulse Dependencies, Classify Spark_System_AutoClassification_com_microsoft_azure_synapse_tokenlib, Spark_User_UserApp_CapacityLimitExceeded, and library management errors, Fix Python not Found error and fluent logger incompatibility with pip in pull request pipeline template, Fix code token scrubber, etc.|
|1403289|LibraryMetadataCooker:2.0.1|Use new LMC version to enable stubgen in spark33|Enable stubgen in Library Metadata Cooker to generate stub files.|
|1406338|DSCopilotInstaller:1.0.0|Including the DS Copilot in the VHD|Making the DS Copilot available as pre-installed in the Fabric environments.|
|1397963|CosmosDBConnector:2.1.1|Update to Cosmos OLAP Connector version 2.1.1, Cosmos OLTP Connector 4.33.0|Update to Cosmos OLAP Connector version 2.1.1, Cosmos OLTP Connector 4.33.0|
|1400653|FabricDWConnector:1.0.5|Fabric Spark DW Connector - Version 1.0.5 - Spark 3.3.0|Incremental Fabric Spark DW Connector release with support for JDBC data source configuration options to tune read requests, and supportability items|
|1384083|Impulse:1.2.3|Migrate Impulse component library to ADO Feed|Updated Impulse component library to consume from ADO.|
|1396780|TridentTokenLibrary:1.9.4|SPN + Certificate based authentication|New commands enabling SPN + Certificate based authentication|
|1400254|OneLakeClientStarter:spark33/1.0.22|Redirection, performance and security improvements for Onelake|Redirection support for S3 storage, metadata, dataverse shortcuts.|

# Bug Fixes

|Id|Component|Title|Description|
|-----|-----|-----|-----|
|1421104|OneLakeClientStarter:1.0.20, Autotune:1.11.1.2|Regression Hotfixes|Revert a onelake update with regression and fix autotune jar issues|
|1381643|TridentCore1.2.9|Fix for whitespaces in config values in trident-core|This fixes a bug in trident-core where config values with whitespaces are not written and propagated correctly. Because of this issue, onelake catalog does not work with workspaces containing whitespaces.|
|1382189|SparkRPCHistoryServer|Migrate RPCHS artifact to ADO feed & security issue fix|Migrate RPCHS artifact from fileshare to ADO feed. Fix security issues.|
|1396780|TridentTokenLibrary:1.9.4|SPN + Certificate based authentication|New commands enabling SPN + Certificate based authentication|
