# System Environment
*   **Vhd Release**: Spark3.4-Rel-2024-05-08.8-rc.1
...
|-----|-----|-----|-----|
|1285574|Delta:2.4.0.11|Adding automatic setting of 'VOrder.enabled' Delta table property to true and enhancing the Optimize command by introducing a ‘DRY RUN’ option|By default, the Delta engine automatically sets the table property ‘vorder.enabled’ to true during table write operations (such as write, optimize, merge, etc.) if it is not already explicitly set to true or false.|

# Improvements

|Id|Component|Title|Description|
|-----|-----|-----|-----|
|1353902|FabricDWConnector:1.0.4, DWConnector:1.0.1|Fabric Spark DW Connector - Version 1.0.4 - Spark 3.4.1|Enable Fabric DW Connector Version 1.0.4 to all PBI Environment Types.|
|1335377|Conda:8.0.15|Update semantic link to 0.7.2|Install updated semantic-link-sempy (v0.7.2) version to the default Fabric 1.2 runtime for machine learning.|
|1345351|SparkNativeParquetWriter:0.15.1|Update SparkNativeParquetWriter to 0.15.1|Update SparkNativeParquetWriter to 0.15.1 which contains various improvements, including better task failure handling|
|1340531|AutoscaleProbe:3.9.1|Release Autoscale Version 3.9.1|New Autoscale Version contains bugfix for TJS update cores failure and improved cluster snapshot monitoring interval.|
|1342282|Delta:2.4.0.12|Logging Improvements|Improvement: Internal logging improvements.|
|1343329|CSparkMetricUpdate:1.0.2|Internal Logging improvements|Internal Logging improvements|
|1345659|Delta:2.4.0.13|Performance improvements to Delta when Velox is enabled|Optimize UPDATE and MERGE commands when Velox is enabled.|
|1285574|Delta:2.4.0.11|Adding automatic setting of 'VOrder.enabled' Delta table property to true and enhancing the Optimize command by introducing a ‘DRY RUN’ option|By default, the Delta engine automatically sets the table property ‘vorder.enabled’ to true during table write operations (such as write, optimize, merge, etc.) if it is not already explicitly set to true or false.|
|1324579|OnelakeClientStarter:1.0.19.4|OnelakeClientStarter additional telemetry|Add export of OnelakeClientStarter startup telemetry.|
|1340417|Wildfire/spark34:1.9.7|New spark 3.4 stack release|New spark 3.4 stack release, support special character in table names, cred leak fixes, new spark event to tract customer submit job time. Cred leak fixes in livy, changes in livy repl.|
|1320280|SparkAdvisor:0.0.17|Update advisor to 0.0.17|Add query Signature and autotune related configuration for each Spark SQL query based on its historical data|
|1322755|SparkDiagnosticsLibrary:1.0.21|Add SparkDiagnosticsLibrary changes for runtime spark 3.4|Add  spark3.4 version of runtimeSparkDiagnosticsLibrary and enable log analytics in spark3.4.|
...
|1335377|Conda:8.0.15|Update semantic link to 0.7.2|Install updated semantic-link-sempy (v0.7.2) version to the default Fabric 1.2 runtime for machine learning.|
|1340531|AutoscaleProbe:3.9.1|Release Autoscale Version 3.9.1|New Autoscale Version contains bugfix for TJS update cores failure and improved cluster snapshot monitoring interval.|
|1341210|Wildfire:1.9.7|Certificate Handling Changes in Sovereign Clouds|Changes to certificate installation and handling for Mariner VHDs in Sovereign Clouds|
|1342282|Delta:2.4.0.12|Logging Improvements|Improvement: Internal logging improvements.|
|1334259|ServiceConfigurationTemplates:spark/3.4.0|Dependency changes in Livy and yarn-nm service startup|Ensure system services are started in correct order so that components are ready before job is submitted.|
|1338730|Autotune:1.11.1.0|Enable autotune by setting spark config from notebook cell|There was a bug to enable autotune by setting spark config from notebook cell. This change contains the fix for this use case.|
|1340417|Wildfire/spark34:1.9.7|New spark 3.4 stack release|New spark 3.4 stack release, support special character in table names, cred leak fixes, new spark event to tract customer submit job time. Cred leak fixes in livy, changes in livy repl.|
...
|Name|Version|
|-----|-----|
|**AutoscaleProbe**|**3.9.0 --> 3.9.1**|
|**Autotune**|**1.11.0.0 --> 1.11.1.0**|
|AzureMLExtensions|1.0.3|
|**CSparkMetricUpdate**|**1.0.1 --> 1.0.2**|
|**Conda**|**8.0.14 --> 8.0.15**|
|CosmosDBConnector|1.8.10|
|**DWConnector**|**1.0.0 --> 1.0.1**|
|**Delta**|**2.4.0.9 --> 2.4.0.13**|
...
|**OneLakeClientStarter**|**1.0.19.3 --> 1.0.19.4**|
...
|**SparkAdvisor**|**0.0.16 --> 0.0.17**|
|SparkDiagnosticsLibrary|1.0.21|
|SparkLighter|2.0.8|
|**SparkNativeParquetWriter**|**0.12.1 --> 0.15.1**|
...
|seaborn|0.12.2|
|seaborn-base|0.12.2|
|**semantic-link-sempy**|**0.6.0 --> 0.7.2**|
|send2trash|1.8.2|
...
|**SparkCustomEvents**|**3.4.0-1.0.1 --> 3.4.0-1.0.2**|
...
|**autotune-client**|**2.12-1.11.0-3.4-120040110 --> 2.12-1.11.1-3.4-125200601**|
|**autotune-common**|**2.12-1.11.0-3.4-120040110 --> 2.12-1.11.1-3.4-125200601**|
|avro|1.11.3|
|avro-ipc|1.11.3|
|avro-mapred|1.11.3|
...
|**delta-core**|**2.12-2.4.0.9 --> 2.12-2.4.0.13**|
|**delta-iceberg**|**2.12-2.4.0.9 --> 2.12-2.4.0.13**|
|**delta-storage**|**2.4.0.9 --> 2.4.0.13**|
|derby|0.14.2.0|
...
|**gluten-velox-bundle-spark**|**3.4_2.12-mariner_2.0-1.2.0-SNAPSHOT --> 3.4_2.12-mariner_2.0_x86_64-1.2.0-SNAPSHOT**|
|gson|2.8.9|
|guava|4.0.1|
|**hadoop-aliyun**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-annotations**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-aws**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-azure**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-azure-datalake**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|hadoop-azureml|1.0-fs|
|**hadoop-client-api**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-client-runtime**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-cloud-storage**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hadoop-openstack**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|hadoop-shaded-guava|1.1.1|
|**hadoop-yarn-server-web-proxy**|**3.3.4.5.3-122540985 --> 3.3.4.5.3.20240428.1**|
|**hdinsight-spark-metrics**|**3.4.0-1.0.1 --> 3.4.0-1.0.2**|
|hive-common|2.3.9|
...
|**spark-avro**|**2.12-3.4.1 --> 2.12-3.4.1.5.3.20240428.1**|
|spark-avro|2.12-3.4.1|
|**spark-catalyst**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-core**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-enhancement**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|spark-enhancementui|2.12-3.3.0|
|**spark-graphx**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-hadoop-cloud**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-hive**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|spark-kusto-synapse-connector|3.1_2.12-1.3.4|
|**spark-kvstore**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-launcher**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|spark-lighter-contract|2.12-2.0.8_spark-3.4.0|
|spark-lighter-core|2.12-2.0.8_spark-3.4.0|
|**spark-microsoft-tools**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-mllib**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-mllib-local**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-network-common**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-network-shuffle**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-repl**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-sketch**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-sql**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-sql-kafka**|**0_2.12-3.4.1.5.3-122540985 --> 0_2.12-3.4.1.5.3.20240428.1**|
|**spark-streaming**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-streaming-kafka**|**2.12-3.4.1.5.3-122540985 --> 0_2.12-3.4.1.5.3.20240428.1**|
|**spark-streaming-kafka**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-tags**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-token-provider-kafka**|**0_2.12-3.4.1.5.3-122540985 --> 0_2.12-3.4.1.5.3.20240428.1**|
|**spark-unsafe**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|**spark-yarn**|**2.12-3.4.1.5.3-122540985 --> 2.12-3.4.1.5.3.20240428.1**|
|spark_diagnostic_cli|2.1.1_spark-3.4.1|
|sparklyr-connector|1.0.0_spark-3.4.1|
|**sparknativeparquetwriter**|**2.12-0.12.1 --> 2.12-0.15.1**|
|spire|2.12-0.17.0|
...
|transaction-api|1.1|
|trident-core|1.2.6|
|tridentsystemtokenlibrary-assembly|1.6.5|
|tridenttokenlibrary-assembly|1.6.5|
...
|xz|1.9|
|**zookeeper**|**3.6.3.5.3-122540985 --> 3.6.3.5.3.20240428.1**|
|**zookeeper-jute**|**3.6.3.5.3-122540985 --> 3.6.3.5.3.20240428.1**|
|zstd-jni|1.5.2-5|
