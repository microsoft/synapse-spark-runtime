# System Environment
*   **Vhd Release**: Spark3.4-Rel-2024-02-07.1-rc.1
...
|Id|Component|Title|Description|
|-----|-----|-----|-----|
|1272679|Vegas:spark34/3.4.08.5|Parquet corruption fixes when Intelligent cache is enabled..|Fixed edge when intelligent cache is enabled, where parquet file corruption might occur if multiple tasks on same host access same parquet file concurrently.|
|1270484|NodeServices:1.3.3|Update Node Services to build 020724.0008|Resolves issues with Spark cluster timeouts due to missing VM metadata tags during startup.|
|1244507|Vegas:spark34:3.4.08.4, VhdSetup:spark:1.1.9|Intelligent Cache Security bug fix|Intelligent Cache Security bug fixes.|
|1259060|ComponentName:ComponentVersion|Fix nfs startup issue|Fixes the issue of nfs startup failure caused by incorrect IP address.|
|1269048|Vegas:spark34/3.4.08.5|Parquet corruption fixes when Intelligent cache is enabled..|Fixed edge when intelligent cache is enabled, where parquet file corruption might occur if multiple tasks on same host access parquet files concurrently.|
...
|**spark-sql**|**2.12-3.4.1.5.3-113429500**|**2.12-3.4.1.5.3-117503204**|
