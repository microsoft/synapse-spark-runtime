# System Environment
*   **Vhd Release**: Spark3.3-Rel-2024-01-17.0-rc.1
...
|-----|-----|-----|-----|
|1229306|Wildfire/spark33:1.7.46|SPARK-37932 and SPARK-39983 in spark. Livy: start livy after olc and use dedicated pools for stopping sessions|SPARK-37932 and SPARK-39983 in spark. Livy: start livy after olc and use dedicated pools for stopping sessions.|

# Improvements

|Id|Component|Title|Description|
|-----|-----|-----|-----|
|1230555|Spark3.3:Conda:1.0.17|Fix statsmodels import issue, support async create experiment/model|Fix dead lock when importing statsmodels in spark3.3 runtime. Improve experiment and model artifact create reliability by doing async create.|
|1227748|GenevaAgents:Various|Update log settings to emit Spark ApplicationMaster logs every 1 minute|Update the priority and duration of Geneva configurations to emit Spark ApplicationMaster logs every 1 minute.|
|1199833|NodeServices/spark33:1.2.1, VhdSetup/Finalize/spark3:1.1.0|Disable SSHD service by default|Disable SSHD service by default, update NodeServices to enable the SSHD service in development environments.|
|1218732|Impulse:1.2.0|Impulse Release 1.0.18|Fix Impulse Listener Memory Leak, improve error code classification and ensure PII and EUII is scrubbed from Microsoft's internal logs.|
|1222718|VhdSetup/Initialize/spark:1.1.7|Update DNS Block List|This adds additional domains to the DNS block list.|
|1231447|NotebookUtils:1.1.38|Print R inline installation log to log file instead of notebook stdout|We only print R inline installation log to log file instead of notebook output.|
|1171111|Wildfire/spark33:1.7.41|Increase spark.nonjvm.error.buffer.size for better error classification|We employ error messages to categorize error codes. In the context of using PySpark, the configuration parameter, spark.nonjvm.error.buffer.size, determines the size of the message used for error classification. It's worth noting that certain error scenarios, such as stack overflow errors, may necessitate a larger amount of log data to accurately classify the issue.|
|1206688|VhdSetup/Initialize/spark:1.1.6, AutoscaleProbe/spark33:3.7.1, NodeServices/spark33:1.2.1, ServiceConfigurationTemplates/spark34:spark/3.3.0|Fix permissions for session token directory|This PR fixes permissions for the session token directory to allow other services to access.|

# Bug Fixes

|Id|Component|Title|Description|
|-----|-----|-----|-----|
|1228163|ServiceConfigurationTemplates:spark/3.3.0|Redact exposed fs.azure.sas token|Redact exposed fs.azure.sas token|
|1230710|SparkInitialize:1.0.0|Upgrade libstdc++6 to the latest version|With this change we are upgrading libstdc to the latest version to support scenarios requiring higher versions of GLIBCXX|
|1230555|Spark3.3:Conda:1.0.17|Fix statsmodels import issue, support async create experiment/model|Fix dead lock when importing statsmodels in spark3.3 runtime. Improve experiment and model artifact create reliability by doing async create.|
|1218732|Impulse:1.2.0|Impulse Release 1.0.18|Fix Impulse Listener Memory Leak, improve error code classification and ensure PII and EUII is scrubbed from Microsoft's internal logs.|
|1244513|VhdSetup/Initialize/spark:1.1.9|Remove top level domains from DNS blocklist|Remove top level domains from DNS blocklist.|
|1244364|TridentCore:1.1.41|Make the session id available for Autoscale scenarios for Spark Batch Jobs|Make the Fabric Job service session id available for Autoscale scenarios for Spark Batch Jobs.|
|1225623|LibraryManagement:1.0.21|New Library Management release with security fixes|This new release covers multiple security fixes|
|1224619|Wildfire/spark33:1.7.44|Enabling Dedicated Thread Pool for Cancelling Session|Cancelling a job can be delayed because it's thread pool is shared with session creation. This change will make Livy to use dedicate thread pool so that cancelling can be done regardless how many session creation requests are there.|
|1187229|SparkFinalize/spark33:1.0.6, Wildfire/spark33:1.7.46|Library management fix for DEP workspace 3.3|This PR fixes a regression that happens on DEP workspace 3.4 when adding python libraries|
...
|**hadoop-azure-datalake**|**3.3.3.5.2-108696741**|**3.3.3.5.2-112613907**|
...
|**spark-core**|**2.12-3.3.1.5.2-108696741**|**2.12-3.3.1.5.2-112613907**|
