...
*   **Vhd Release**: Spark3.5-Rel-2024-08-28.3-rc.1
...
|-----|-----|-----|
|1401457|ServiceConfigurationTemplates:spark/3.5.0|By default, the Delta engine automatically sets the table property ‘vorder.enabled’ to true during table write operations (such as write, optimize, merge, etc.) if it is not already explicitly set to true or false.|
|1444584|TridentCore:1.2.16, Wildfire/spark35:1.10.7|Enables schema feature on Spark 3.5, fixes bug for spark sql queries on file when onelake spark catalog is enabled with no attached lakehouses to the session, and fixes a leak in driver logs when custom configuration is provided|
|1432823|TokenLibrary:4.3.0, ServiceConfigurationTemplates:core-site.xml.liquid|Enabling Reading and Writing Internal Log Files to SystemStorage account with shorter lived tokens|
|1440492|TridentCore:1.2.15|* Add schema feature to Spark 3.5|


# Improvements
|Id|Component|Description|
|-----|-----|-----|
|1446557|Wildfire:1.10.9|Updating the latest bigtop to resolve driver log issue.|
|1443101|Delta:3.2.0.5|Delta 3.2.0.5 includes the following improvements: Low Shuffle Merge, the merge command performance optimization that has been existed in the previous versions, is completely rewritten for Delta 3.2. LSM in Delta 3.2 improves merge performance for target tables with deletion vectors enabled as well. Improves statistics collection performance when writing Delta tables by reading statistics from written Parquet files when possible. FSCK command now supports handling missing deletion vectors. Columnar optimize write for native execution engine is ported from Delta 2.4.|
|1443185|ServiceConfigurationTemplates:Spark35|Changing the default value of spark.sql.parquet.outputTimestampType configuration option to TIMESTAMP_MICROS (https://spark.apache.org/docs/latest/sql-data-sources-parquet.html).|
|1445033|Wildfire:1.10.8|Releasing latest wildfire-spark changes.|
|1440217|ServiceConfigurationTemplates:spark/3.5.0|We onboard a new feature in Livy that can expose the debug jupyter messages from kernel. In BBC runtime, we add a feature flag to control this feature by using Livy conf.|
|1441574|TridentTokenLibrary:1.9.5|Retries for IO errors when invoking TokenProvider|
|1441782|NotebookUtils:1.1.52|Set no-symlinks=true to Improve Blobfusev2 performance|
|1416734|Wildfire/Spark35:1.10.5|Onboarded on a security mechanism so no SAS tokens to system storage accounts are valid longer than 1 hour.|
|1432058|NotebookUtils:1.1.51|Upgrade NotebookUtils to 1.1.3, improve help api doc, enable feature flag for Data Wrangler display entry by default, fix runMultiple "Index 962 out of bounds for length 13" issue, add notebookutils.notebook.getDefinition and improve notebookutils.notebook.updateDefinition|
|1434394|Conda:5.0.0, FsspecWrapper:1.0.3, LibraryManager:1.0.4, Python:3.11/1.0.1, R:1.0.6|Upgrade python runtime with Anaconda and R version to 4.4.1 in Fabric Spark3.5 runtime|
|1441428|TokenLibrary:4.3.2|Thwart burst mode access to TokenService|


# Bug Fixes
|Id|Component|Description|
|-----|-----|-----|
|1451705|TokenLibrary:4.2.2|Reverting change to fix regression with Managed VNet session startup|
|1444584|TridentCore:1.2.16, Wildfire/spark35:1.10.7|Enables schema feature on Spark 3.5, fixes bug for spark sql queries on file when onelake spark catalog is enabled with no attached lakehouses to the session, and fixes a leak in driver logs when custom configuration is provided|
|1429418|LibraryMetadataCooker:2.0.2|Previously the jar metadata cooker version is incorrect, causing error in Library Management job.|
|1431227|CSparkMetricUpdate:1.0.5|Upgrade org.json jar to handle security vulnerability in Spark 3.5.|
|1432058|NotebookUtils:1.1.51|Upgrade NotebookUtils to 1.1.3, improve help api doc, enable feature flag for Data Wrangler display entry by default, fix runMultiple "Index 962 out of bounds for length 13" issue, add notebookutils.notebook.getDefinition and improve notebookutils.notebook.updateDefinition|
|1440492|TridentCore:1.2.15|* Add schema feature to Spark 3.5|
...
|adlfs|2024.2.0|**matplotlib-base**|**3.8.3 ⬆️ 3.8.0**|
...
|**azure-core**|**1.29.4 ⬆️ 1.30.2**|**ninja**|**1.10.2**|
...
|**cairo**|**1.18.0 ⬆️ 1.16.0**|**path.py**|**12.5.0**|
...
|**fonttools**|**4.53.0 ⬆️ 4.25.0**|pytorch-mutex|1.0|
...
|**jsonpointer**|**3.0.0 ⬆️ 2.1**|sqlanalyticsfabricconnectorpy|1.0.0|
...
|**libcurl**|**8.8.0 ⬆️ 8.5.0**|torchmetrics|1.4.0.post0|
...
|**libxcb**|**1.16 ⬆️ 1.15**|yaml-cpp|0.8.0|
...
|jackson-dataformat-xml|2.12.7|**spark-sql**|**2.12-3.5.1.5.4.20240722.1 ⬆️ 2.12-3.5.1.5.4.20240830.3**|
...
|diagram|1.6.5|**roxygen2**|**7.3.1 ⬆️ 7.3.2**|
...
|**gt**|**0.10.1 ⬆️ 0.11.0**|timechange|0.3.0|
